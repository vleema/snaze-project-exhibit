...
public:
  ...
  [[nodiscard]] bool found_food(const Position &pos) const {
    return pos == m_food;
  }
  ...
  str_in_game(const std::deque<Position> &snake_body,
...
private:
  ...
  size_t m_height{
      10}; //!< The height of the maze array, i.e. his number of rows.
  size_t m_width{
      10}; //!< The width of the maze array, i.e. his number of lines.
  ...
  std::vector<Position>
      m_free_cells; //!< Used for more efficiently generating a food position
