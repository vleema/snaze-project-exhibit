...
#include <utility>
namespace snaze {
SnakeBot::MaybeDirectionDeque SnakeBot::solve(const Maze &maze,
                                              const Snake &snake) {
  ...
  while (not to_visit.empty()) {
    ...
    for (const auto &dir : directions) {
      ...
      if (maze.in_bound(next_pos) and
          not(maze.blocked(next_pos, Direction::None)) and
...
std::vector<Direction> SnakeBot::positions_available(const Maze &maze,
                                                     const Snake &snake) {
...
SnakeBot::MaybeDirectionDeque SnakeBot::play_random(const Maze &maze,
  ...
  auto head_dir = available_moves[std::experimental::randint(
      0, (int)available_moves.size() - 1)];
