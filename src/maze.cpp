...
std::string Maze::str_debug(const std::deque<Direction> &solution,
  ...
  for (const auto &row : maze_copy) {
    for (const auto &cell : row) {
      if (cell == Cell::Free or cell == Cell::InvisibleWall or
          cell == Cell::Spawn) {
        oss << free;
      } else if (cell == Cell::Wall) {
...
std::string Maze::str_in_game(const std::deque<Position> &snake_body,
  ...
  maze_copy[snake_body.front().coord_y][snake_body.front().coord_x] =
      Cell::SnakeHead;
  for (const auto &row : maze_copy) {
    for (const auto &cell : row) {
      if (cell == Cell::Free or cell == Cell::InvisibleWall or
...
void Maze::random_food_position() {
  ...
  m_food = m_free_cells[std::experimental::randint(
      0, (int)(m_free_cells.size() - 1))];
