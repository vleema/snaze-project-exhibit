  ...
  if (not m_snake_bot.solution.has_value()) {
    ...
    if (!m_snake_bot.solution.has_value()) {
      throw std::runtime_error(
          "Something went wrong, while the bot was thinking");
    }
  }
}
...
std::string SnazeManager::game_loop_info() const {
  ...
  for (size_t i = 0; i < (size_t)(m_settings.lives - m_remaining_snake_lives);
       ++i) {
    header_oss << " " << skull;
  }
  header_oss << " | Score: " << m_score << "| Food eaten "
             << m_eaten_food_amount_snake << " of " << m_settings.food_amount
             << std::endl
             << line(line_length) << std::endl;
...
Position SnazeManager::update_snake_position() {
  ...
  case Direction::Down:
    ...
    break;
  case Direction::Left:
    m_snake.body.emplace_front((m_snake.body[0].coord_x - 1 % m_maze.width()) %
                                   m_maze.width(),
